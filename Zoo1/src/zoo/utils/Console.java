...
public class Console {
	...
	public static boolean readYesOrNo(String prompt) {
		...
		if (!pattern.matcher(line.trim()).matches()) return false;
	...
	public static Money readMoney(String prompt) {
		...
		return new Money(readLine("%s (money is in %s and is given like \"100.%s\") > ", 
				prompt, symbol, Strings.repeatChar('0', decimals).trim()));
	}
