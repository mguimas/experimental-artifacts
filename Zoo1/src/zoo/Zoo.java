...
public final class Zoo {
	...
	/**
	 * The number of bought but not yet used tickets.
	 */
	private int availableTickets = 0;
	
	private Money ticketPrice = new Money("10.00");
	
	private Money balance = new Money("5000.00");
	...
	private boolean overlaps(Area area) {
		for (Zone zone : zones) 
			if (area.overlaps(zone.getArea())) return true;
		return false;
	}
	...
	public Class[] getSpeciesWithoutZone() {
		...
		for (Zone zone : Zoo.INSTANCE.getZones())
	...
	public int useTickets(int tickets) throws ZooException {
	...
	/**
	 * The Zoo application starts here.
	 * @param args
