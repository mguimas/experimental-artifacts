...
class FinanceMenu extends Menu {
	...
	private static class TicketPriceOption extends MenuOption {
		...
		public void run() {
			...
			String line = System.console().readLine("What will be the new price? > ");
			try {
				Money newPrice = new Money(line);
				if (newPrice.isPlus()) {
				...
				System.out.println("Price not changed: " + e.getMessage());
			}
		}
	}
	...
	private static class DonateOption extends MenuOption {
		...
		public void run() {
			...
			try {
				...
				if (donation.isPlus()) {
					Zoo.INSTANCE.setBalance(balance.plus(donation));
