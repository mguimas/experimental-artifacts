...
class MaintenanceMenu extends Menu {
	...
	private static class RemoveZoneOption extends MenuOption {
		...
		public void run() {
			...
			if (zone != null) {
				...
				if (Console.readYesOrNo(
						"Are you sure you want to remove the " + species + " zone?")) {
					try {
						Zoo.INSTANCE.removeZone(zone);
						System.out.println("Zone for " + species + "s has been removed!");
					} catch (ZooException e) {
						System.out.println("Zone for " + species 
								+ "s could not be removed because it is not empty");
					}
				}
			}
		}
	...
	private static class AddAnimalOption extends MenuOption {
		...
		public void run() {
			...
			if (zone != null) {
				zone.addAnimal();
				System.out.println("You added a " + zone.getSpeciesName().toLowerCase());
			}
		}
	}
