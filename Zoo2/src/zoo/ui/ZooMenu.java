...
public final class ZooMenu extends Menu {
		...
		public void run() {
			try {
				...
				if (numTickets > 0) {
					...
					System.out.println("You now have " + Zoo.INSTANCE.getTicketsHoldByVisitors() + " tickets");
				...
				System.out.println("Type a valid number next time!");
			}
		}
	}
	...
	private class VisitZooOption extends MenuOption {
		...
		public void run() {
			...
			int available = Zoo.INSTANCE.getTicketsHoldByVisitors();
			if (available == 0) {
			...
			try {
				int numTickets = Console.readInteger("You have " + available 
						+ " tickets. How many visitors are you? > ");
				if (numTickets > 0) {
					Zoo.INSTANCE.acceptTickets(numTickets);
					available = Zoo.INSTANCE.getTicketsHoldByVisitors();
				...
				System.out.println("You only have " + Zoo.INSTANCE.getTicketsHoldByVisitors() 
